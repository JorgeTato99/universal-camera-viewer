        ...
        if db_cameras:
            ...
            for cam in db_cameras:
                ...
                camera_info = CameraInfo(
                    ...
                    ip_address=cam.ip,
                    port=(
                        cam.connection_config.rtsp_port
                        if hasattr(cam.connection_config, "rtsp_port")
                        else 554
                    ),
                    protocol=(
                        cam.protocol.value
                        if hasattr(cam, "protocol") and cam.protocol
                        else "RTSP"
                    ),
            ...
            return create_response(
                success=True, data=[camera.model_dump() for camera in cameras]
            )
    ...
    try:
        ...
        if db_camera:
            ...
            camera_info = CameraInfo(
                ...
                port=(
                    db_camera.connection_config.rtsp_port
                    if hasattr(db_camera.connection_config, "rtsp_port")
                    else 554
                ),
                protocol=(
                    db_camera.protocol.value
                    if hasattr(db_camera, "protocol") and db_camera.protocol
                    else "RTSP"
                ),
                status=state.get("status", "disconnected"),
                is_connected=state.get("is_connected", False),
...
async def reset_camera_states():
    ...
    return create_response(
        success=True, data={"message": "Estados de cámaras reseteados exitosamente"}
    )
